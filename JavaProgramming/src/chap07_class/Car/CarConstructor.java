...
public class CarConstructor {
	...
	// 생성자는 관례적으로 속성과 기능 사이에 정의한다.
	// 접근제어자 + 클래스명(매개변수) 으로 지정
	// 매개변수가 없는 생성자를 기본생성자라고 하며 생성자를 정의하지 않았을 때 기본으로 제공된다.
	// 생성자는 객체를 생성하면서 속성들을 초기화할 때 주로 사용한다.
	public CarConstructor() {
		// 생성된 객체들의 company는 모두 현대로 지정된 다음에 생성된다.
		company = "현대";
	}
	...
	// 매개변수가 생성자에 어떤 변수를 초기화하는 명확하게 하기 위해서 속성 변수명과 일치시키는게 관례적이다.	
	// 매개벼수가 있는 생성자를 생성하면 기본생성자가 제공되지 않기 때문에 항상 기본생성자를 먼저 정의를 해주고 다른 생성자들을 정의한다.

...
import chap21_multithread.thread.PrintThread;
...
public class _14_ThreadPool02 {
	...
	public static void main(String[] args) {
		...
		// 2. 스레드 풀의 스레드들은 데못 스레드가 아니기 때문에 
		// 주 스레드가 종료되도 남아있는 작업들은 계속해서 진행된다.
		...
		// shutdown()는 대키큐에 남아있는 스레드들이 계속 작업이 진행되기 때문에 
		// 문제가 발생할 수 있다. 따라서 shutdown() 메소드로도 스레드 풀의 모든
		// 작업 종료될 수 있도록 boolean 변수를 하나 선언해서.
		// shutdown() 메소드가 호출됐음을 스레드에 알리고 스레드가 안정하게 종료될 수 있다.
		threadPool.shutdown();
		pt.setShutDown(true);

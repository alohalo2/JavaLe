...
public class _07_StringBuffer02 {
	...
	public static void main(String[] args) {
		...
		// 버퍼의 크기인 16보다 긴 문자열을 append 하면 자동으로 문자열의 길이에 맞춰서 버퍼의 크기가 증가한다.
		...
		// deleteCharAt 메소드의 매개변수가 하나일 때는 해당 인덱스의 문자 하나만 삭제
		sb2.append("camp")
			.deleteCharAt(3);
		System.out.println(sb2);
		
		// 5. insert: 첫 번쨰 매개변수 위치에 두 번쨰 값을 삽입
		sb2.insert(3, "c");
		
		System.out.println(sb2);
		
		// 6. reverse: StringBuffer에 저장되어 있는 문자열을 역순으로 정렬
		StringBuffer sb5 = sb2.reverse(); // 새로운 StringBuffer를 만들어 사용해도 됨, return이 StringBuffer이기 때문에
		
		System.out.println(sb2);
		
		
		// 7. setCharAt: 첫 번째 매개변수로 주어진 인덱스 위치에 있는 문자를 두 번째 매개변수로 주어진 값을 변경한다.
		sb2.setCharAt(2, 'u');
		...
		// 저장되어 있는 길이보다 매매변수가 작을 떄는 저장되어 있는 문자열에서 남은 부분은 삭제
		// 버퍼의 크기는 변경되지 않는다.
		sb2.setLength(20);
		sb1.setLength(20);

...
//			intSet.add(intArr2[i]);
//		}
		
		// 배열을 Stream으로 만들려면 Arrays를 사용
		...
		// 최종단계 메소드
		// reduce(누적값의 초기값, (누적값이 될 변수, 요소에 하나씩 좁군헐 변수, 요소의 인덱스) -> 누적값에 누적할 실행코드);
		// 요소의 인덱스는 선택값으로 써도되고 안써도 된다.
...
//						  .intValue();
		...
		System.out.println(df.format((double)sum2));
	}

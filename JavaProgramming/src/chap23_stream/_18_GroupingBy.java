...
public class _18_GroupingBy {
	...
	public static void main(String[] args) {
		...
		// Map<String, Long>
		// counting은 그냥 개수를 세는 메소드여서 매매변수가 필요없다.
		
		Map<String, Long> companyCntMap = creditCardList.stream()
											  .collect(
													  Collectors.groupingBy(cardCompany -> cardCompany.getCompany(),
														Collectors.counting() 
														)
													  );
											  
		System.out.println(companyCntMap);
		
//		creditCardList.parallelStream(); // Stream 병렬처리

package chap99_homework.homework08.answer;
...
//		1-1. 2001년 12월 19일이 결혼기념일인 부부가 있다. 
//	    윤년이 있는 년도에는 결혼기념일의 날짜를 하루씩 더하기로 했다.
//	    사용자가 입력한 년도에 해당하는 결혼기념일의 날짜를 구하세요.
		
		Calendar cal4 = Calendar.getInstance();
		cal4.set(2001, 11, 19);
		
		Scanner sc1 = new Scanner(System.in);
		System.out.println("알고싶은 결혼기념일 년도를 입력하세요.");
		
		int userInput = sc1.nextInt();
		int diff1 = userInput - 2001;
		int result;
		
		if(diff1 > 0) {
			result = diff1 + 2001;
		} else {
			System.out.println("잘못 입력했습니다, 2001년도 이후로 다시 입력해주세요.");
		}
		
		for(int i = 2001; i < userInput; i++) {
			if(isLeap(i)) {
		...
		Date d2 = new Date(cal4.getTimeInMillis());
		
		System.out.println(userInput + "결혼기념일 날짜는 " 
				+ sdf3.format(d2)
				+ "입니다.");

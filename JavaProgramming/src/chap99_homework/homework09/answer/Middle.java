...
//		1. 학생이름을 key로 시험점수를 value로 가지는 Map<String, Integer>를 생성하고 4명의 학생 데이터를 입력한 후 4명 중 최고 점을 맞은 학생의 이름과 점수을 출력하세요.
		...
		String name = ""; // key 값 출력을 위한 변수 선언
		int score = 0; // value 값 출력을 위한 변수 선언
	
		
		while(it.hasNext()) { // stMap에 index 번째 key와 value값을 불러오기 위한 설정
...
//	key, value의 곱이 가장 큰 Entry를 리턴하는 메소드 getMaxMul을 구현하세요.
	public static Entry<Integer, Integer> getMaxMul(Map<Integer, Integer> map) {
		...
		// map.entry() => Set<Entry<Integer, Integer>>
		for(Entry<Integer, Integer> ent : map.entrySet()) {
			if(num < ent.getKey().intValue() * ent.getValue().intValue()) { // 곱한 값이 최소값보다 작으면
				num = ent.getKey().intValue() * ent.getValue().intValue(); // 곱한 값이 최소값이 된다.
				maxEntry = ent; // 그러면 최대값은 
			}
		}

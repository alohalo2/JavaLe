...
// 인터페이스의 메소드는 추상메소드이기 때문에 구현부({})를 만들 수 없다.
...
public interface Board {
	...
	// 변수는 static final로 지정되기 때문에 static 키워드에 의해서 메모리에 저장될 떄 값이 저장이 되어 있지 않으면 안되기 때문에
	// 선언 시 값을 지정하지 않으면 에러가 발생한다.
	int num = 10;
	
	// 인터페이스에서 정의된 모든 메소드는 추상메소드이기 때문에 구현부({}블록)을 만들면 에러가 발생한다.
//	void post() {
...
//	}
	...
	// default 메소드는 구현부를 만들 수 있기 때문에 상속받은 자식클래스에서 구현하지 않아도 된다.

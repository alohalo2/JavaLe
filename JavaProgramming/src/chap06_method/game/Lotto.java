...
public class Lotto {
	...
	// 1. 1~45까지 숫자 중 랜던 값 7개를 저장해서 배열로 리턴하는 메소드 generateLottoArray를 구현하세요.
	// 매개변수 없음
	...
	public int[] generateLottoArray() {
		...
		for(int i = 1; i < lotto.length; i++) {
			lotto[i] = (int)(Math.random() * 45) + 1; // Math.random() (0 <=  < 1) 사이에 수를 뽑아내기 때문에
			                                          // 45를 곱하면 (0 <=    < 45)로 0을 포함하게 된다.
												      // 따라서 1을 더해줌으로서 (1 <=    < 46)이 되어 1~45 숫자가 됨   

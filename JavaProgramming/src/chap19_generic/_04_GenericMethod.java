...
//		_04_GenericMethod.<Integer>getLastElement2(new int[] {1, 2, 3, 4, 5});
	...
	// 1. 제네릭 메소드 정의
	// 리턴타입 앞에 <T> 타입 매개변수를 붙여서 리턴 타입이나 매개변수의 타입이 열려있는
	// 메소드를 정의할 수 있다.
...
//	public static <T> T getLastElement2(int[] intArr) {
//		T t = null;
...
//	}
	...
	public static <K, V> int getSumMap(Map<K, V> map) { // key, value 모두 사용하고 싶을때는 <K, V>
		                                                // value만 사용하고 싶을때는 <V>
		int sum = 0;
	...
	public static <K, V> K getMaxMap(Map<K, V> map) {
		...
		K maxKey = null; // key, value를 사용할 경우 각각 변수 선언 필요
		V maxValue = null;
		
		int index = 0;
		
		for(Entry<K, V> entry : map.entrySet()) {
			if(index == 0) { // 처음은 그냥 저장
				maxKey = entry.getKey();
				maxValue = entry.getValue();
			} else {
				if((int) entry.getValue() > (int) maxValue) { // maxValue가 entry.getValue()보다 작을때가 최대값으로 생각
					maxKey = entry.getKey();
					maxValue = entry.getValue();
				}
			}
